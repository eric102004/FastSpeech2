...
def CG(params: List[Tensor],
    ...
    o_loss = outer_loss(params, hparams)
    '''
    #check if requires_grad is True
    print('o_loss:', o_loss.requires_grad)
    for idx, w in enumerate(params):
        if w.requires_grad==False:
            print(f'params no grad! idx:{idx}')
    for idx, hw in enumerate(hparams):
        if hw.requires_grad==False:
            print(f'hparams no grad! idx:{idx}')
    print(done)
    '''
    grad_outer_w, grad_outer_hparams = get_outer_gradients(o_loss, params, hparams)
